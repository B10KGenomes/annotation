...
for my $name(keys %ref){
	for my $id(sort {$ref{$name}{$a}{mRNA}->[3] <=> $ref{$name}{$b}{mRNA}->[3]} keys %{$ref{$name}}){
		...
		for(@aa){
			...
			printf "\t%g\t%.2f\t%.2f", $mRNA_over,($mRNA_over/($q->{mRNA}->[4]-$q->{mRNA}->[3]+1)),($mRNA_over/($p->{mRNA}->[4]-$p->{mRNA}->[3]+1));
			printf "\t%g\t%g\t%g\t%.2f\t%.2f\n", $len_q_cds,$len_p_cds,$cds_over,($cds_over/$len_q_cds),($cds_over/$len_p_cds);
		}
	}
}
